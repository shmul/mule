...
if _VERSION >= 'Lua 5.2' then
  _ENV = lunit.module('test_MinutelyAlg','seeall')
else
  module( "test_MinutelyAlg", lunit.testcase,package.seeall )
end
...
function test_minute()
  ...
  for line in io.lines('./tests/fixtures/graphs_of_interest.txt') do
    local tokens = {}
    local index = 0
      ...
      if(zz > 0) then
        ...
        for key,value in pairs(result) do
          if(value[2]) then
						local td = key-1
						tdind = tdind + 1
						assert_equal(matlab[zz][tdind][1],td, string.format('failed test alarm: signal num = %d, alert num = %d, got %d instead of %d', zz, tdind, td, matlab[zz][tdind][1]))
						assert_equal(matlab[zz][tdind][2],math.floor(value[3]+0.5), string.format('failed test estimated: signal num = %d, alert num = %d, got %d instead of %d', zz, tdind, math.floor(value[3]+0.5), matlab[zz][tdind][2]))
						assert_equal(matlab[zz][tdind][3],value[4], string.format('failed test ano-measure: signal num = %d, alert num = %d, got %d instead of %d', zz, tdind, value[4], matlab[zz][tdind][3]))
          end
        end
        assert_equal(#matlab[zz],tdind, string.format('failed test: signal num = %d, entries num = %d, expected = %d',zz, tdind, #matlab[zz]))
      end
    end
  end
end
