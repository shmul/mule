...
function test_top_level_factories()
  ...
  local m = mule(in_memory_db())
  helper(m)
  local bdb = cabinet_db_factory("./tests/temp/top_level")
  m = mule(bdb)
  helper(m)
end
...
function test_modify_factories_3()
  local bdb = cabinet_db_factory("./tests/temp/modify_factories")
  local m = mule(bdb)
  helper_modify_factories(m)
end
...
function test_reset()
  ...
  local m = mule(in_memory_db())
  helper(m)
  local bdb = cabinet_db_factory("./tests/temp/reset")
  m = mule(bdb)
  helper(m)
end
...
function test_process_tokyo()
  local bdb = cabinet_db_factory("./tests/temp/process_tokyo")
  local m = mule(bdb)
  m.configure(table_itr({"beer.ale 60s:12h 1h:30d","beer.stout 3m:1h","beer.wheat 10m:1y"}))
  assert_equal(0,#m.matching_sequences("beer.ale"))
  local factories = m.get_factories()
...
function test_latest()
  os.remove("./tests/temp/process")
  local bdb = cabinet_db_factory("./tests/temp/process")
  local m = mule(bdb)
...
function test_update_only_relevant()
  local bdb = cabinet_db_factory("./tests/temp/update_only_relevant")
  local m = mule(bdb)
...
function test_metric_one_level_childs()
  local bdb = cabinet_db_factory("./tests/temp/one_level_childs")
  local m = mule(bdb)
  ...
  for j,t in ipairs(tests) do
	...
	for i in one_level_childs(bdb,t[1]) do
...
function test_dump_restore()
  ...
  local bdb = cabinet_db_factory("./tests/temp/dump_restore")
  local m = mule(bdb)
