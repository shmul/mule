...
** TODO "undefined" keeps on scrolling at the status bar -> 100% progress bar zdoes't go off
...
** TODO Graph auto-refresh should be visible to the user
   Also, it would be nice to have it "togglable".
** TODO Enable closing the legend without changing which graphs(subkeys) are displayed
...
** TODO The name of the currently showed graph should be very visible, including the current retention
