...
** TODO The tool tip over the stack graph shows the % sign with no value
** TODO Times are shown in local time, right? Should be UTC
...
** TODO The x-axis contains full time stamps which can be too much for the eye. What about showing the day only when they change?
...
** TODO When presenting single graphs (i.e. not stacked) the lines should be thicker.
** TODO Add an option to reset the zoom factor
** DONE Toggle Legend
